...
local Window = library:CreateWindow("SHOP by @boo10001", Vector2.new(550, 680), Enum.KeyCode.RightShift)
...
-- ==================== SEEDS TAB ====================
local SeedsTab = Window:CreateTab("Shop Seeds")
...
seedSelectedToggle = SeedsSector:AddToggle("Auto Buy Selected Seeds", autoBuySelectedSeeds, function(Value)
...
-- ==================== GEAR TAB ====================
local GearTab = Window:CreateTab("Shop Gear")
...
gearSelectedToggle = GearSector:AddToggle("Auto Buy Selected Gears", autoBuySelectedGears, function(Value)
...
-- ==================== CRATE TAB (with spaces) ====================
local CrateTab = Window:CreateTab("Shop Crate")
...
crateDropdown = CrateSector:AddDropdown("Select Crate", {
    ...
    "Weather Machine Crate",
    "Wood Wall Crate"
}, "", true, function(Value)
    selectedCrates = type(Value) == "table" and Value or {Value}
end)

crateSelectedToggle = CrateSector:AddToggle("Auto Buy Selected Crates", autoBuySelectedCrates, function(Value)
...
CleanupSector:AddButton("Run Cleanup + Smooth + 15 FPS", function()
    cleanupEnabled = true
    fpsCap = 15
    applyLowCPU()
...
CleanupSector:AddButton("Set FPS Cap 30", function()
    fpsCap = 30
    local setfpscap = setfpscap or function() end
    setfpscap(30)
...
CleanupSector:AddButton("Set FPS Cap 60", function()
    fpsCap = 60
    local setfpscap = setfpscap or function() end
    setfpscap(60)
...
local configDropdown = ConfigSector:AddDropdown("Choose Config to Load", getConfigList(), currentConfigName, false, function(Value)
...
ConfigSector:AddButton("Load Selected Config", function()
...
ConfigSector:AddButton("Refresh Config List", function()
    print("=== Configs in BOOSCRIPT ===")
    for _, name in pairs(getConfigList()) do
        print(name)
        pcall(function() configDropdown:Add(name) end)
    end
...
ControlSector:AddButton("X - Destroy GUI", function()
...
local function createRestoreCircle()
    ...
    circle.Position = UDim2.new(0, 20, 0.5, -27)
...
ControlSector:AddButton("- Hide GUI", function()
    hideGUI()
end)

ControlSector:AddButton("Show GUI", function()
    showGUI()
end)
...
print("SHOP by @boo10001 loaded - Crates with spaces")
