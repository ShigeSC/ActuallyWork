...
function Library.new(title, options)
    ...
    self.ScreenGui = create("ScreenGui", {
        ...
        DisplayOrder = 99999,  -- High priority
        Parent = playerGui,
    })
...
local isLoadingConfig = false  -- Prevent double-loading

if not isfolder(CONFIG_FOLDER) then
    makefolder(CONFIG_FOLDER)
end

-- Variables (forced boolean false)
local selectedSeeds = {}
local selectedGears = {}
...
-- UI References (will be set when UI is created)
local seedDropdown, gearDropdown, crateDropdown
local seedSelectedToggle, seedAllToggle, gearSelectedToggle, gearAllToggle
local crateSelectedToggle, crateAllToggle, configDropdownObj, configNameTextbox


-- Helper: Force boolean conversion
local function toBool(val)
    return val == true or val == 1 or (type(val) == "string" and val:lower() == "true")
end

-- Apply Low CPU settings function (defined early for config system)
local function applyLowCPU()
    if cleanupEnabled then
        local Workspace = game:GetService("Workspace")
        local Lighting = game:GetService("Lighting")

        -- Destroy plants, trees, visual effects
...
local function saveConfig(name)
    if not name or name == "" then
        Window:Notify("Error", "Please enter a config name!", 3)
        return
    end
    ...
    local data = {
        ...
        autoBuySelectedSeeds = autoBuySelectedSeeds == true,  -- Force boolean
        autoBuyAllSeeds = autoBuyAllSeeds == true,
        autoBuySelectedGears = autoBuySelectedGears == true,
        autoBuyAllGears = autoBuyAllGears == true,
        autoBuySelectedCrates = autoBuySelectedCrates == true,
        autoBuyAllCrates = autoBuyAllCrates == true,
        fpsCap = fpsCap,
    ...
    if configDropdownObj and configDropdownObj.Refresh then
        configDropdownObj.Refresh(getConfigList())
    end
    
    Window:Notify("Config Saved", "Saved as: " .. name, 3)
    print("[CONFIG] Saved: " .. name)
end

local function applyConfigToUI()
    print("[DEBUG] Applying config to UI...")
    
    -- Update dropdowns - USE DOT NOT DOT COLON
    if seedDropdown and seedDropdown.Set then 
        pcall(function() seedDropdown.Set(selectedSeeds) end)  -- . not :
    end
    if gearDropdown and gearDropdown.Set then 
        pcall(function() gearDropdown.Set(selectedGears) end) 
    end
    if crateDropdown and crateDropdown.Set then 
        pcall(function() crateDropdown.Set(selectedCrates) end) 
    end

    -- Update toggles - USE DOT NOTATION
    if seedSelectedToggle and seedSelectedToggle.Set then 
        pcall(function() seedSelectedToggle.Set(toBool(autoBuySelectedSeeds)) end) 
    end
    if seedAllToggle and seedAllToggle.Set then 
        pcall(function() seedAllToggle.Set(toBool(autoBuyAllSeeds)) end) 
    end
    if gearSelectedToggle and gearSelectedToggle.Set then 
        pcall(function() gearSelectedToggle.Set(toBool(autoBuySelectedGears)) end) 
    end
    if gearAllToggle and gearAllToggle.Set then 
        pcall(function() gearAllToggle.Set(toBool(autoBuyAllGears)) end) 
    end
    if crateSelectedToggle and crateSelectedToggle.Set then 
        pcall(function() crateSelectedToggle.Set(toBool(autoBuySelectedCrates)) end) 
    end
    if crateAllToggle and crateAllToggle.Set then 
        pcall(function() crateAllToggle.Set(toBool(autoBuyAllCrates)) end) 
    end
...
local function loadConfig(name)
    if isLoadingConfig then 
        print("[CONFIG] Already loading, skipping...")
        return false 
    end
    
    if not name or name == "" then 
        print("[CONFIG] No name provided")
        return false 
    end
    
    local path = CONFIG_FOLDER .. "/" .. name .. ".json"
    if not isfile(path) then
        Window:Notify("Config Error", "Config not found: " .. name, 3)
        return false
    end

    isLoadingConfig = true
    print("[CONFIG] Loading: " .. name)
    ...
    if success and data then
        ...
        autoBuySelectedSeeds = toBool(data.autoBuySelectedSeeds)
        autoBuyAllSeeds = toBool(data.autoBuyAllSeeds)
        autoBuySelectedGears = toBool(data.autoBuySelectedGears)
        autoBuyAllGears = toBool(data.autoBuyAllGears)
        autoBuySelectedCrates = toBool(data.autoBuySelectedCrates)
        autoBuyAllCrates = toBool(data.autoBuyAllCrates)
        
        fpsCap = data.fpsCap or 15
        cleanupEnabled = toBool(data.cleanupEnabled)
        currentConfigName = name

        print("[CONFIG] Values loaded:")
        print("  autoBuySelectedSeeds:", autoBuySelectedSeeds)

        -- Apply to UI (updates seeds/gear/crate dropdowns and toggles)
        applyConfigToUI()
        
        -- Update config UI to show loaded config (ADD THIS HERE)
        if configNameTextbox and configNameTextbox.Set then
            configNameTextbox.Set(name)
        end
        if configDropdownObj and configDropdownObj.Set then
            configDropdownObj.Set(name)
        end
        
        -- Apply Low CPU settings
        applyLowCPU()

        -- Save as last used
        writefile(CONFIG_FOLDER .. "/last.txt", name)
        
        Window:Notify("Config Loaded", "Loaded: " .. name, 3)
        print("[CONFIG] Successfully loaded: " .. name)
    else
        Window:Notify("Config Error", "Failed to parse config file", 3)
        print("[CONFIG] Error parsing:", success, data)
    end
...
local function deleteConfig(name)
    ...
    if isfile(path) then
        delfile(path)
        Window:Notify("Config Deleted", "Deleted: " .. name, 2)
        -- Refresh dropdown
        if configDropdownObj and configDropdownObj.Refresh then
            configDropdownObj.Refresh(getConfigList())
        end
        -- Clear textbox if it matches deleted config
        if currentConfigName == name and configNameTextbox and configNameTextbox.Set then
...
configNameTextbox = ConfigTab:AddTextbox("Config Name", currentConfigName, function(text)
...
-- DROPDOWN: Only selects name, does NOT auto-load
configDropdownObj = ConfigTab:AddDropdown("Choose Config", configOptions, currentConfigName, function(choice)
    ...
    print("[CONFIG] Selected:", choice)
    -- REMOVED: loadConfig(choice) - Don't auto-load on select!
end)
...
-- LOAD BUTTON: Only place where loadConfig is called manually
ConfigTab:AddButton("Load Config", function()
    local nameToLoad = selectedConfigName or currentConfigName
    if nameToLoad and nameToLoad ~= "" then
        print("[CONFIG] Manual load triggered for:", nameToLoad)
        loadConfig(nameToLoad)
    else
        Window:Notify("Error", "Please select or type a config name first!", 3)
    end
end)
...
ConfigTab:AddButton("Refresh Config List", function()
    ...
    Window:Notify("Config List", "Found " .. #newList .. " config(s).", 2)
end)
...
task.spawn(function()
    ...
    local lastConfigPath = CONFIG_FOLDER .. "/last.txt"
    if isfile(lastConfigPath) then
        local lastConfig = readfile(lastConfigPath)
        if lastConfig and lastConfig ~= "" then
            print("[CONFIG] Auto-loading last config:", lastConfig)
